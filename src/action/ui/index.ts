...
export const moveToSelectedTask = () => {
  State.update(state => {
    ...
    const finishTime = new Date(currentTime.getTime() + (viewportWidth / columnWidth * State.Option.scaleTime(scale)));

    if (startTime.getTime() > target.finishedAt.getTime()) {
      state.ui.currentTime = new Date(target.startedAt.getTime() - State.Option.scaleTime(scale) * 14);
